...
function tsfproject_plugin_action_links( $actions, $plugin_file, $plugin_data, $context ) {
	$actions['widgets'] = '<a href="'. admin_url('widgets.php') .'">'. __( 'Widgets', '365project-widget' ) .'</a>';
	return $actions;
}
...
class tsfproject_widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'tsfproject-widget', 'description' => __( 'Display images from 365project.org feeds', '365project-widget' ) );
		$control_ops = array( 'width' => 300 );
		parent::WP_Widget( 'tsfproject_widget', __( '365Project.org Widget', '365project-widget' ), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<p style="width:63%;float:left;">
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', '365project-widget' );?>
		...
		<p style="width:33%;float:right;padding-top:20px;height:20px;">
			...
			<label for="<?php echo $this->get_field_id('hide_title'); ?>"><?php _e('Hide Title?', '365project-widget' );?></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'feed' ); ?>"><?php _e( 'Feed:', '365project-widget' );?>
			...
			<span class="description"><?php printf( __( 'e.g. %s', '365project-widget' ), '<code>http://365project.org/browse/latest/feed</code>' ); ?></span>
		</p>
		<p style="width:48%;float:left;">
			<label for="<?php echo $this->get_field_id( 'timeout' ); ?>"><?php _e( 'Timeout (in seconds):', '365project-widget' );?>
			...
			<span class="description"><?php _e( 'How long should the widget be cached?', '365project-widget' ); ?></span>
		</p>
		<p style="width:48%;float:right;">
			<label for="<?php echo $this->get_field_id( 'limit' ); ?>"><?php _e( 'Limit:', '365project-widget' );?>
				<input class="widefat" id="<?php echo $this->get_field_id('limit'); ?>" name="<?php echo $this->get_field_name('limit'); ?>" type="text" value="<?php echo $limit; ?>" />
			</label>
			<span class="description"><?php _e( 'How many items to show?', '365project-widget' ); ?></span>
		</p>
		<?php
	} //end form()
}